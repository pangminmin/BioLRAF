...
"""BioLRAF single-sample 3D ternary visualization example.

Input requirements
------------------
The input CSV must contain cell-level BioLRAF scores:
    Os, Ch, Ad, MSC, celltype

This script calculates normalized lineage-associated activities,
MSC_ratio, Diff_score, and lineage_dominant. By default, it uses the complete
dataset to export two 3D ternary plots: one colored by celltype and one colored
by lineage_dominant. Each plot is exported as interactive HTML and four
fixed-view PNG files.
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Calculate BioLRAF metrics and export 3D ternary plots."
    )
    parser.add_argument("--input", required=True, help="Cell-level BioLRAF score CSV.")
    parser.add_argument("--output-dir", required=True, help="Output directory.")
    parser.add_argument(
        "--dataset-label",
        default="BioLRAF_example",
        help="Dataset label used in titles and output filenames.",
    )
    parser.add_argument(
        "--celltypes",
        nargs="*",
        default=None,
        help="Cell types to plot. Omit to process every cell type.",
    )
    parser.add_argument(
        "--mode",
        choices=["total", "by-celltype", "both"],
        default="total",
        help=(
            "Plot the complete dataset, separate cell types, or both. "
            "Default: total."
        ),
    )
    parser.add_argument(
        "--center-delta",
        type=float,
        default=DEFAULT_CENTER_DELTA,
        help="Tolerance around 1/3 used to define Non-dominant cells.",
    )
    return parser
...
def main() -> None:
    ...
    scores = read_score_table(input_path)
    scores = calculate_biolraf_metrics(scores, center_delta=args.center_delta)
    output_dir.mkdir(parents=True, exist_ok=True)

    processed_path = output_dir / (
        f"{safe_filename(args.dataset_label)}_BioLRAF_scores_with_states.csv"
    )
    scores.to_csv(processed_path, index=False)
    ...
    if args.mode in {"by-celltype", "both"}:
        export_celltype_plots(
            df=scores,
            output_dir=output_dir / "by_celltype",
            dataset_label=args.dataset_label,
            celltypes=args.celltypes,
        )

    print("\nAll requested BioLRAF plots have been processed.")
